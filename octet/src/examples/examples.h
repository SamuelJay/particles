...
#include "particle_system/particle_app.h"
...
static app *app_factory(const char *name, int argc, char **argv) {
  ...
  else if (!strcmp(name, "particle")) return new particle_app(argc, argv);//  invaderers sample: continue here to learn how to make sounds
...
inline void run_examples(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    app *myapp = app_factory("particle", argc, argv);
